...
func StopBlockProfile() {
	...
	if err = pprof.Lookup("block").WriteTo(f, 0); err != nil {
		log.Fatal("can't write %s: %s", filename, err)
	}
	f.Close()
}

func SerMemProfileRate(rate int) {
...
func DumpHeap() {
	filename := "heap-" + strconv.Itoa(pid) + "-" + strconv.Itoa(int(atomic.AddInt32(&headpPorfileCounter, 1))) + ".pprof"
	f, err := os.Create(filename)
	if err != nil {
		fmt.Fprintln(os.Stderr, "testing: can't write %s： %s", filename, err)
